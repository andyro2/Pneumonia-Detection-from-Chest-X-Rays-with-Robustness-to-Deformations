...
def test():
    ...
    # dirs = {'train': data_dir+train_dir,
            # 'val': data_dir+valid_dir,
            # 'test': data_dir+test_dir
            # }
    # image_datasets = {x: datasets.ImageFolder(dirs[x], transform=data_transforms[x]) for x in
    #                   ['train', 'valid', 'test']}
    # load the data into batches
    # dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=16, shuffle=True) for x in
    #                ['train', 'valid', 'test']}
    ...
    # dataloaders = {x: torch.utils.data.DataLoader(dset[x],drop_last=True, batch_size=256, shuffle=True, num_workers=num_threads)
    #                for x in categories}
    #
    # inputs, classes = next(iter(dataloaders["train"]))
    # out = torchvision.utils.make_grid(inputs)
    # class_names = dirs["train"].classes
